# -*- coding: utf-8 -*-
"""root_builder.py —— 用新的 word_family() 重新计算所有词库的词族字段。"""
import os, re, json
...
def main():
    ...
    for bid in books:
        js_path = os.path.join(BOOKS_DIR, bid + '.js')
        ...
        for ent in words:
            ...
            fam = word_family(w)
            ent['root'] = fam; n += 1
        new_code = 'window.BOOK_%s = %s;' % (bid, json.dumps(obj, ensure_ascii=False))
        with open(js_path, 'w', encoding='utf-8') as f:
            f.write(new_code)
        pct = round(n / len(words) * 100) if words else 0
        print(f'  {bid:10s} -> {len(words)} 词 | 词族={n} ({pct}%)')
