...
(function () {
  ...
  var CHANGELOG = [
    { ver: "20260730r", note: "记忆曲线恢复7天一档 + 更新弹窗仅新版触发" },
  ...
  // 兜底：如果上方定义缺失（缓存旧版），从全局读取
  if (typeof FULL_BOOKS === 'undefined' && window.FULL_BOOKS) { var FULL_BOOKS = window.FULL_BOOKS; }
...
function fillAnalysis(node, bw) {
  ...
  function renderReview() {
    ...
    if (!reviewQueue.length || reviewIdx >= reviewQueue.length) { stage.innerHTML = '<div class="panel" style="text-align:center">今日复习全部完成，明天见！</div>'; return; }
